...
3. **Score each dimension** — apply the rubrics from the evaluation criteria reference, in this order:
  - Score **Usage Value**: assign five sub-scores (`problem_clarity`, `audience_breadth`, `capability_gap`, `actionability`, `reusability`), assign `domain_calibration_bonus` (0.0–0.7), and compute final 1-5 score
...
|-----------|------|--------|
| **Usage Value** | Weighted (1-5) + 5 sub-scores + domain bonus | How useful is this skill to users after accounting for clarity, breadth, capability gap, actionability, reusability, and specialization bonus? |
...
- By default, missing evaluations are treated as an error (fail-fast) to prevent "not evaluated" skills in the output.
- By default, duplicate skill names from scan data are deduped deterministically (first occurrence kept).
- Use `--allow-missing-evaluations` or `--allow-duplicate-names` only when explicitly needed.
